        ...
        <form action="agendamento?action=${not empty agendamento ? 'atualizar' : 'inserir'}" method="POST">
            ...
            <div class="card-body p-4">
                ...
                <fieldset class="border p-3 mb-4">
                    <legend class="float-none w-auto px-2 fs-6">Passo 1: Detalhes do Agendamento</legend>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label for="advogado" class="form-label">Advogado Responsável:</label>
                        <input type="text" class="form-control" value="<c:out value='${advogadoLogado.nome}'/>" disabled readonly>
                    </div>
                </fieldset>
                ...
                <fieldset class="border p-3 mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="data" name="data" required value="${agendamento.data}" min="${hoje}" <c:if test="${isPastAppointment}">readonly</c:if> disabled>
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label">Horários Disponíveis:</label>
                        <div id="horarios-container" class="border p-3 rounded bg-light">
                            <p class="text-muted m-0">Por favor, selecione um cliente para habilitar a data.</p>
                        </div>
                ...
                <fieldset class="border p-3 mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="descricao" name="descricao" rows="4" required><c:out value="${agendamento.descricao}"/></textarea>
                    </div>
                    <div class="mb-3">
...
<script>
    $(document).ready(function() {
        ...
        var horaInput = $('#hora');
        var idAdvogado = $('input[name="idAdvogado"]').val(); 

        clienteSelect.on('change', function() {
            var idCliente = $(this).val();
            dataInput.prop('disabled', !idCliente).val('');
        ...
        dataInput.on('change', function() {
            ...
            horariosContainer.html('<div class="d-flex justify-content-center align-items-center h-100"><div class="spinner-border text-primary" role="status"></div><span class="ms-2 text-primary">Verificando horários...</span></div>');
            $.ajax({
